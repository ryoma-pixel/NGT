...
- **トップの絵巻は診断の結果そのものに変更**（ご本人指示「どん！と絵巻が出てきて、その続きで工程に進む」「下の絵巻は不要」）。第12回の `#night`（秘境神社の絵巻）と「See its route」ボタンは廃止。
  ...
  - 流れ：3問に答える → 煙がぼっ → 封をした巻物（任務）→ 押すと紐がほどけ、巻物が下へ沈んで診断の欄が畳まれる → 画面が絵巻へ移動 → 集中線と「ドン！」で絵巻が落ちてきて揺れる → 自動で開いて、スクロールで行程の場面を読み進める。
  - 表紙の下に選んだ理由（For 浅草 · 夜 · 神社）・料金（幅＋ドル目安）・「Book this mission」（予約エンジンへ直接、準備中のツアーは「Ask about dates」→お問い合わせ）。
  - 「Try again」で絵巻は再び隠れる（`ngt:mission-reset`）。移動のスクロールは、上の診断が畳まれている間も着地点を追いかける（スマホでずれないように）。
  - 動きを減らす設定では、演出なしで絵巻を表示して移動するだけ（横スワイプの帯）。
- **殿と忍者のやりとり**：質問は殿の吹き出し（「The Lord asks · 01」）。答えると忍者が頷きながら返事をする（「Umu, umu. Asakusa. Noted.」など）。結果は殿の「It is decided. Untie the scroll, traveler.」。
  - 殿・忍者の顔は、いまは明朝の「殿」「忍」の丸バッジで仮置き。キャラクターのイラスト（浮世絵風の将軍と忍者）は進め方の承認待ち。できたらバッジと差し替える。

...
- 行程：**忍者版ファミコン風の3Dステージ**（`NinjaStage.astro`、ご本人指示「3D、ファミコンのマリオの忍者版」）。スクロールに合わせてステージが固定され、ドット絵の忍者が走る。行程ごとの3Dブロック（アイコンは行程名から自動、`src/lib/step-icon.ts`）を通過するとブロックが跳ね、小判が飛び出してカード（分数・行程名）が開く。上部にHUD（NINJA×01／WORLD 地名／TIME 経過分）、遠景のドットのビル街と雲はパララックス、ゴールは旗と鳥居で「Mission clear!」。ドット文字は Press Start 2P（OFL、`public/fonts/press-start-2p-latin.woff2` 約7KB）。動きを減らす設定・JSなしでは横にスワイプできる普通の帯で全カード表示。任天堂の素材（？ブロック・キャラ・土管・音）は使わない。
...
- Googleの評価と件数の自動更新：`scripts/update-ratings.ts`＋`.github/workflows/daily-ratings.yml`（毎朝、Places API (New)。変わったときだけコミット）。必要：シークレット `GOOGLE_MAPS_API_KEY`、変数 `GOOGLE_PLACE_ID`（初回実行で候補が出る）。口コミ本文は手動で選ぶ（ご本人方針：動機づけできるものを選ぶ）。Tripadvisor はRIDEの掲載のみのため手動。

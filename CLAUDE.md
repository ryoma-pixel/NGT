...
- 地図：旧サイトの埋め込みの座標は「表示範囲の中心」でピンとずれていたため、公式サイトの埋め込みの場所ID（cid）でリンクし直し、集合場所の写真の横に地図を埋め込み（`meetingPoint.mapEmbed`）。ハンコは公式サイトでも本文＝バスタ新宿／地図＝新宿東宝ビルと食い違っているため、本文どおりバスタ新宿（要確認）。
- 行程：ルートマップ型（アイコンの停留所・大きな分数・画面に入ると道が光り手裏剣が走る）。アイコンは行程名から自動（`src/lib/step-icon.ts`）。
...
- 規約・特商法・プライバシー（ご本人回答 2026-09-25：キャンセル24時間前まで無料／メールは info_ninja-go-ride@ninja-kabukitokyo.com のまま／住所・責任者は変更なし／電話は「請求があれば遅滞なく開示」／年齢制限なし・飲酒20歳以上・雨天決行・荒天は全額返金・遅刻不参加は返金なし／英語版のみ）で書き直し。ページ上部に「DRAFT · under legal review」（frontmatter `reviewDraft: true`）。**公開前に法務確認し、施行日を入れて `reviewDraft` を外す**。ウォークツアーの保険の有無は未確認のため記載していない（Eスクーターの保険はPart Bに残した）。
